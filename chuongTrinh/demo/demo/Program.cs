    ...
    class Program
    {
        static void Main(string[] args)
        {
			...
			for (int f = 0; f < inp.Length; f++)
			{
				...
				string t1 = string.Format("\nTotal time of Eratosthenes {0}: {1} ticks = {2}s", f+1, tEra.ElapsedTicks, Math.Round(1.0 * (tEra.ElapsedTicks * Math.Pow(10,-8)), 4));
				...
				string t2 = string.Format("\nTotal time of Sundaram {0}: {1} ticks = {2}s", f+1, tSun.ElapsedTicks, Math.Round(1.0 * (tSun.ElapsedTicks * Math.Pow(10,-8)), 4));
